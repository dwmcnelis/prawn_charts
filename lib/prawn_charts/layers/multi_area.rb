  ...
  module Layers
    ...
    class MultiArea < Layer
      ...
      def draw(pdf, coords, options={})
        ...
        draw_poly(pdf, coords, area_color, options = {})
        ...
        if @baselines
          ...
          @baselines.each do |baseline|
            ...
            draw_poly(pdf, lower_poly_coords, area_color, options = {})
          end
        end
      end

      private
      def draw_poly(pdf, coords, color, options={})
        points_value1 = "0,#{height} #{stringify_coords(coords).join(' ')} #{width},#{height}"
        pdf.polygon(:points => points_value1, :fill => color.to_s, :stroke => color.to_s, 'style' => "opacity: #{opacity}")
      end
