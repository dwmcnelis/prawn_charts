    ...
    class Box < Layer
      ...
      def draw(pdf, coords, options = {})
        coords.each_with_index do |coord,idx|
          ...
          pdf.line(:x1=>x+@bar_width/4,:x2=>x+@bar_width/4*3,:y1=>y[4],:y2=>y[4], :style => "stroke:#{(outline.to_s || options[:theme].marker || 'white').to_s}; stroke-width:1")
          pdf.rect( :x => x, :y => y[1], :width => @bar_width, :height => (y[1]-y[3])*-1,
            :fill => color.to_s, 'style' => "opacity: #{opacity}; stroke:#{(outline.to_s || options[:theme].marker || 'white').to_s}; stroke-width:1;" )
          pdf.line(:x1=>x,:x2=>x+@bar_width,:y1=>y[2],:y2=>y[2], :style => "stroke:#{(outline.to_s || options[:theme].marker || 'white').to_s}; stroke-width:1")
