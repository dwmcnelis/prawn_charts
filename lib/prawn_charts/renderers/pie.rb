  ...
  module Renderers
    ...
    class Pie < Renderer
      ...
      def initialize
        ...
        self.components << PrawnCharts::Components::Graphs.new(:graphs, :position => [0, 15], :size => [70, 85])
        self.components << PrawnCharts::Components::Title.new(:title, :position => [5, 2], :size => [90, 7])
        self.components << PrawnCharts::Components::Legend.new(:legend, :position => [70, 15], :size => [30, 85], :vertical_legend => true)
      end
    end # Pie
